...
if not dfs:
    st.warning("No sheets found. Ensure BHI.xlsx is in the same folder or upload a file."); st.stop()
...
if po_input == "":
    st.info("Type a PO number above to see related invoices."); st.stop()

# Filter invoices by PO
if "PO_NUMBER" not in Invoices.columns:
    st.error("Invoices sheet missing 'PO_NUMBER' column."); st.stop()

inv_for_po = Invoices[Invoices["PO_NUMBER"].astype(str).str.fullmatch(po_input, case=False, na=False)]
if inv_for_po.empty:
    st.warning(f"No invoices found for PO: {po_input}"); st.stop()
...
total_invoiced = _num_series(inv_for_po["INVOICE_AMOUNT"]).sum() if "INVOICE_AMOUNT" in inv_for_po.columns else 0
if "PO_NUMBER" in POs.columns and "PO_AMOUNT" in POs.columns:
    po_row = POs[POs["PO_NUMBER"].astype(str) == po_input]
    if not po_row.empty:
        po_amount = _num_series(pd.Series([po_row.iloc[0]["PO_AMOUNT"]])).iloc[0]
        st.metric("Total Invoiced (this PO)", f"{total_invoiced:,.0f}")
        if pd.notna(po_amount):
            st.metric("PO Amount", f"{po_amount:,.0f}")
            st.metric("Variance", f"{(po_amount - total_invoiced):,.0f}")
